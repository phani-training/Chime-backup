...
//We are using C++ 20 libraries so old depreciated APIs of C++17 will give warnings and erros, so avoid it, we are adding a preprocessor directive called _SILENCE_CXX17_CODECVT_HEADER_DEPRECATION_WARNING;
...
MeetingDetails GetMeetingDetails(Uri request) {
	...
	try
	{
		...
		auto mediaPlacement = jsonValue.GetObject().GetNamedObject(L"Meeting").GetObject().GetNamedObject(L"MediaPlacement");
